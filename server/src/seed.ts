...
export function validatePackagePricing(official: number, sell: number, buy: number): void {
  if (!(buy < sell && sell < official)) {
    throw new Error('Invalid pricing: require buy_price < sell_price < official_price');
  }
}

export async function seedPackages(force = false): Promise<number> {
  const row = await queryOne<{ c: string }>('SELECT COUNT(*)::text AS c FROM packages');
  const count = Number(row?.c ?? 0);
  if (count > 0 && !force) return count;

  if (force) await query('DELETE FROM packages');
  ...
  for (const pkg of DEFAULT_PACKAGES) {
    ...
    await query(
      ...
      [pkg.coin_amount, pkg.official_price_brl, pkg.sell_price_brl, pkg.buy_price_brl, pkg.sort_order],
    );
  }
